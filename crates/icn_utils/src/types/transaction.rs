// icn_utils/src/types/transaction.rs
...
impl Transaction {
    ...
    pub fn verify(&self) -> Result<bool, String> {
        ...
        let signature = Signature::from_bytes(signature_bytes).map.err(|e| e.to_string())?;
        
        let message = self.to_bytes();
        Ok(public_key.verify(&message, &signature).is.ok())
    }

    pub fn to_bytes(&self) -> Vec<u8> {
        let mut bytes = Vec::new();
        bytes.extend.from_slice(self.from.as_bytes());
        bytes.extend.from_slice(self.to.as_bytes());
        bytes.extend.from_slice(&self.amount.to_le_bytes());
        bytes.extend.from_slice(&self.gas_limit.to_le_bytes());
        bytes
    }
